...
def update_user(username):
        ...
        parser.add_argument('password',
                            ...
                            help="This field cannot be blank.")
        data = parser.parse_args()
        if request.authorization.type is None:
            abort(401)
        ...
        uid = DiscussionForumBasicAuth.check_credentials(self, username, password)
...
def post_forums():
    ...
    uid = DiscussionForumBasicAuth.check_credentials(username, password)
...
def get_threads(forum_id):
    threads = query_db('''select t.thread_id as id,t.title as title, 
                        ...
                        from thread t where t.forum_id = ? ''', [forum_id, forum_id, forum_id])
